...
async def metrics_middleware(request: Request, call_next):
    """
    Middleware that records Prometheus HTTP metrics for each request.

    Tracks request count, latency histogram, in-progress gauge,
    and per-user request counters.
    """
...
def register_metrics_middleware(mcp: FastMCP) -> None:
    """Register the Prometheus metrics middleware with the FastMCP application."""
    if hasattr(mcp, "app") and mcp.app:
        mcp.app.middleware("http")(metrics_middleware)
